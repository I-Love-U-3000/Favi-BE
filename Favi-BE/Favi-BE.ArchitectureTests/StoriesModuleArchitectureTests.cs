...
/// <summary>
/// STR-01..STR-06: CQRS segregation, handler internals, and cross-module boundary enforcement
...
public class StoriesModuleArchitectureTests
{
    ...
    private const string CommandsNs  = "Favi_BE.Modules.Stories.Application.Commands";
    private const string QueriesNs   = "Favi_BE.Modules.Stories.Application.Queries";
    private const string WriteModels = "Favi_BE.Modules.Stories.Application.Contracts.WriteModels";

    // STR-01: Command handlers must not import from the Queries namespace.
    [Fact]
    ...
    // STR-02: Query handlers must not import from the Commands namespace.
    [Fact]
    ...
    // STR-03: Query handlers must not depend on WriteModels.
    [Fact]
    ...
    // STR-04: Command handlers must be internal.
    [Fact]
    ...
    // STR-05: Query handlers must be internal.
    [Fact]
    ...
    // STR-06: Stories module must not depend on any other module's application internals.
    [Fact]
    public void Stories_Should_Not_Depend_On_Other_Module_Application_Namespaces()
    {
        var result = Types
            .InAssembly(StoriesAssembly)
            .ShouldNot()
        ...
        result.IsSuccessful.Should().BeTrue(
            because: $"Stories must not import other modules' application namespaces. " +
                     $"Failing types: {string.Join(", ", result.FailingTypeNames ?? [])}");
    }
}
