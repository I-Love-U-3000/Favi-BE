...
/// <summary>
/// MSG-01..MSG-06: CQRS segregation, WriteModels isolation, handler internals,
/// and cross-module boundary enforcement for the Messaging module.
/// </summary>
public class MessagingModuleArchitectureTests
{
    private static readonly Assembly Assembly =
        Favi_BE.Modules.Messaging.AssemblyReference.Assembly;

    private const string CommandsNs  = "Favi_BE.Modules.Messaging.Application.Commands";
    private const string QueriesNs   = "Favi_BE.Modules.Messaging.Application.Queries";
    private const string WriteModels = "Favi_BE.Modules.Messaging.Application.Contracts.WriteModels";

    // MSG-01: Command handlers must not import from the Queries namespace.
    [Fact]
    ...
    // MSG-02: Query handlers must not import from the Commands namespace.
    [Fact]
    ...
    // MSG-03: Query handlers must not depend on WriteModels.
    [Fact]
    ...
    // MSG-04: Command handlers must be internal.
    [Fact]
    ...
    // MSG-05: Query handlers must be internal.
    [Fact]
    ...
    // MSG-06: Messaging module must not depend on any other module's application internals.
    [Fact]
    public void Messaging_Should_Not_Depend_On_Other_Module_Application_Namespaces()
    {
        var result = Types
            .InAssembly(Assembly)
            .ShouldNot()
        ...
        result.IsSuccessful.Should().BeTrue(
            because: $"Messaging must not import other modules' application namespaces. " +
                     $"Failing types: {string.Join(", ", result.FailingTypeNames ?? [])}");
    }
}
